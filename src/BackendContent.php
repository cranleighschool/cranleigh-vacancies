...
class BackendContent {
	...
	function next_removal_notice( $views ) {
		...
		echo '<p><em>The expired vacancy remove script will next run in: <strong>' . human_time_diff(
			time(),
			$timestamp
		) . '</strong>. This will auto remove expired vacancies that have been expired for ' . Configuration::EXPIRY_REMOVAL . ' or more.</em></p>';
	...
	function manage_columns( $column, $post_id ) {
		switch ( $column ) {
			case 'expiry':
				...
				printf( __( '%1$s at %2$s' ), $expiry_date, $expiry_time );
				break;
			case 'photo':
